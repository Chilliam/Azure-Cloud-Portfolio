...
After both projects were live, both web VMs were found stopped one evening
with no immediately obvious cause. Investigation using
`az monitor activity-log list` (filtering on the VM resource IDs and
reviewing the `caller` field) traced the triggering identity to a GUID
that didn't match the Project 5 Automation Account's managed identity.
Resolving that GUID via `az ad sp show` identified it as **Azure Lab
Services' first-party service principal** — the identity that operates
behind the scenes for the `Microsoft.DevTestLab/schedules` resource type,
which is what actually powers the plain VM "Auto-shutdown" feature
configured back in Project 4.

This confirmed two independent shutdown mechanisms were both active and
scheduled for the same 11 PM trigger time: Project 4's auto-shutdown
schedules, and Project 5's `nightly-stop` schedule linked to the
`Stop-DevVMs` runbook. Project 4's mechanism was the one that had actually
fired.

**Resolution:** removed the redundant `nightly-stop` schedule link from
the `Stop-DevVMs` runbook (via the Portal, since schedule-linking has no
reliable CLI path — see the challenges section above), while keeping the
runbook itself intact for on-demand/manual use. Project 4's auto-shutdown
schedules remain the sole automated nightly shutdown mechanism, since
they require no managed identity or role assignment overhead for what is
otherwise the same outcome.

**Why this is worth documenting rather than quietly fixing:** diagnosing
an unexpected VM state change back to its actual root cause — rather than
assuming which automation was responsible — is a realistic Azure Admin
troubleshooting scenario, and `az monitor activity-log list` combined with
`az ad sp show` is a genuinely useful pattern for tracing any unexplained
resource change back to the identity that caused it.
